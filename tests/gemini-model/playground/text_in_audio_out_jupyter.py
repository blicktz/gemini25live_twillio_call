...
# Import our IPython-compatible web display
from jupyter_web_display import display, Markdown, Audio, jupyter_display
...
async def main():
    ...
    print("Starting IPython-compatible web display...")
    jupyter_display.start_server()
    print(f"Web display available at: http://127.0.0.1:{jupyter_display.port}")
    ...
    async with client.aio.live.connect(
        ...
        if audio_data:
            # This works exactly like the original IPython.display.Audio call
            display(Audio(np.concatenate(audio_data), rate=24000, autoplay=True))
            print("Audio generated and available in web interface!")
        
        # Keep the server running so you can interact with the web interface
